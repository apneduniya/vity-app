...
class WalletEncryption {
  ...
  private static readonly encryptionKey = (() => {
    ...
    if (!key) {
      throw new Error('WALLET_ENCRYPTION_KEY environment variable is not defined');
    }
  ...
  static async encrypt(source: string): Promise<string> {
    try {
      ...
      logger('Failed to encrypt private key', error, {
  ...
  static async decrypt(encrypted: string): Promise<string> {
    try {
      ...
      logger('Private key decryption failed', error, {
        module: 'WalletEncryption',
        level: 'error',
      });
